       ...
       <!-- search gproject nav bar starts here -->
      		<div id="gp_nav_bar" class="panel" style="min-height: 70px;" >
          		...
          		<div class="col-sm-6" >
					...
					<button id="dpop" type="button" class="btn btn-outline btn-primary btn-responsive deleteCategory" style="margin-right: 2px; margin-top : 15px; display: none;"><i class="icon wb-trash" aria-hidden="true"></i><span class="hidden-sm hidden-xs" >Delete</span>
					</button>
             	...
             	<?php 
					$c=1;
						foreach ($result as $row){
							...
							echo "<td><i onclick='lef($categoryid)' class='icon wb-edit' aria-hidden='true' ></i></td>";
							echo "<td><i onclick='initiate_delete($categoryid)' class='icon wb-trash' aria-hidden='true' ></i></td>";
							echo "<td><i onclick='view($categoryid)' class='icon wb-eye' aria-hidden='true' ></i></td>";
						  	echo '</tr>';
							$c++;			
						}
				?>   
...
<!--							   <?php //echo form_open_multipart('upload/do_upload');?>-->
								...
								<form method="post" action="<?=base_url()?>Category/add_category" autocomplete="off"/>
								<div class="col-xs-12">
									    <div class="form-group form-material floating row">
										   <div class="col-sm-12">
											<input type="text" class="form-control" id="#" name="categoryname" required/>
											<label class="floating-label">Event Category Name</label>
										  </div>
										</div>  
										<div class="form-group form-material floating row">
						  ...
						  <div class="modal-dialog">
							...
							<div class="modal-content">
							  <div class="modal-header">
								<button type="button" class="close" data-dismiss="modal">&times;</button>
								<h4 class="modal-title">Edit Event Category Data</h4>
							  </div>
							  <div class="modal-body">
							  
							   <form id="updatedevice" method="post" action="<?=base_url()?>Category/update_category" autocomplete="off">
...
<script>
	...
	function lef(a){
		...
		document.getElementById("categoryid").value = a;
		
		var baseurl = "<?php echo base_url(); ?>";
		var xmlhttp = new XMLHttpRequest();
		var url = baseurl + "Category/get_category/" + a;
		...
		function myFunction(response) {
			...
			for (var i = 0; i < arr.length; i++) {
				...
				document.getElementById("categoryname").value = arr[i].category_name;
