            ...
            <div class="card-body">
                <div class="tabs">
                    ...
                    <div class="tab-content">
                        <section class="card">
                            <div class="card-body">
                                <form method="GET" action="<?= base_url('Report/finance_report2')?>" autocomplete="off">
                                ...
                                <div class="col-md-12">
                                    <table class="table table-responsive-md table-striped mb-0">
                                        ...
                                        <tbody>
                                            <?php foreach($revenue_per_streams as $rps):?>
                                            <?php
                                                $area_council_total_revenue = get_revenue_stream_area_council(
                                                    $rps->id, $rps->target, $selected_year,
                                                    $lower_date_bound, $upper_date_bound
                                                );
                                            ?>
                                            <?php
                                                if (in_array($rps->name, $acclusion_array)) {
                                                    continue;
                                                }

                                                $detailsBtn = "";
                                                if(count($area_council_total_revenue) > 0) {
                                                    $detailsBtn="<a href='#' data-id='".$rps->id."' class='more-detail-btn collapsed'><i class='fa fa-plus' aria-hidden='true'></i></a>";
                                                } 
                                            ?>
                                            <tbody class="labels">
                                                <tr>
                                                    <td>
                                                        <div class="container">
                                                            <div class='row'>
                                                                ...
                                                                <div class="col-sm-9">
                                                                    <label for="<?=$rps->id?>"><b><?=$rps->name?></b></label>
                                                                    <input type="checkbox" name="<?=$rps->id?>" id="<?=$rps->id?>" data-toggle="toggle">
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td><b><?=number_format((float)$rps->total_amount, 2, '.', ',');?></b></td>
                                                    <td><b><?=number_format((float)$rps->amount_paid, 2, '.', ',');?></b></td>
                                                    <td>
                                                        <b><?=number_format((float)$rps->total_amount-$rps->amount_paid, 2, '.', ',');?></b>
                                                    </td>        
                                                </tr>
                                            </tbody>
                                            <tbody id='tbody<?=$rps->id?>' class="bill-details hide">
